...
def shell(arg: str, env: Optional[str] = None):
    ...
    if env:
        subprocess.Popen(
            args=["echo", env], stdout=a.stdin, stdin=subprocess.PIPE
        ).communicate()

    subprocess.Popen(
        args=["echo", arg], stdout=a.stdin, stdin=subprocess.PIPE
    ).communicate()
    ...
    if a.stderr.read():
        ...
        raise subprocess.CalledProcessError(
            returncode=a.returncode, cmd=a.args, stderr=a.stderr
        )
        ...
        if mapping[machine] == "x86_64":
            with open("/proc/cpuinfo") as f:
                if "sse4_2" not in f.read():
                    Logger.warning(
                        "x86_64 CPU does not support SSE4.2, falling back to x86..."
                    )
                    return ("x86", 32)
        return mapping[machine]
    raise ValueError("platform.machine '" + machine + "' architecture is not supported")
